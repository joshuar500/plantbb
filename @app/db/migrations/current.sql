...
grant
  select,
  insert (plant_name, last_watered, last_plant_image, comment),
  update (plant_name, last_watered, last_plant_image, comment),
  delete
...
drop table if exists app_public.user_plant_images cascade;

--
-- USER PLANT IMAGES
--

create table app_public.user_plant_images (
  id                serial primary key,
  user_id           uuid default app_public.current_user_id() references app_public.users(id) on delete set null,
  plant_image       text,
  created_at        timestamptz not null default now(),
...
grant
  ...
  update (plant_image),
  delete
on app_public.user_plant_images to :DATABASE_VISITOR;

create policy select_all on app_public.user_plant_images for select using (true);
create policy manage_own on app_public.user_plant_images for all using (user_id = app_public.current_user_id());
